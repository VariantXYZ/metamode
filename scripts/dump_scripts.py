...
COMMANDS[0x0F] = generate_dummy_command("Unknown0F", 1)
    ...
    for idx, struct in enumerate(structs):
        ...
        with open(script_name, "w") as script_set_src:
            ...
            while i < len(code_data[0]):
                command = COMMANDS[code_data[0][i]]
                i += 1
                script_set_src.write(f"  {command.name} ")
                script_set_src.write(",".join([f"{x:02X}" for x in code_data[0][i:i+command.parameter_size]]))
